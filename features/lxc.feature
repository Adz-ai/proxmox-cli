Feature: LXC Container Management
  ...
  Scenario: Create an LXC container from a YAML spec
    Given a valid LXC YAML spec file with the following content:
      """
  ...
  Scenario: List all LXC containers
    Given there are LXC containers on the cluster
  ...
  Scenario: Start a stopped LXC container
    Given an LXC container with ID 200 is stopped
  ...
  Scenario: Stop a running LXC container
    Given an LXC container with ID 200 is running
  ...
  Scenario: Delete an LXC container
    Given an LXC container with ID 200 exists
  ...
  Scenario: List snapshots of an LXC container
    Given an LXC container with ID 200 has snapshots
