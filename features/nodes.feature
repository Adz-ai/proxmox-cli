Feature: Node Management
  ...
  Scenario: List all nodes in the cluster
    Given a Proxmox cluster with multiple nodes
  ...
  Scenario: Describe a specific node
    Given a node named "pve" exists in the cluster
  ...
  Scenario: List storage on a node
    Given a node named "pve" exists with configured storage
  ...
  Scenario: List tasks on a node
    Given a node named "pve" has running tasks
  ...
  Scenario: List only running tasks
    Given a node named "pve" has both running and completed tasks
  ...
  Scenario: Restart a service on a node
    Given a service named "pveproxy" exists on node "pve"
